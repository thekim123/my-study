어제 로그인 기능에 관리자 계정에 IP를 지정하고 그 IP가 아니면 접속을 못하게 하는 기능을 만드는데 잘 되지 않았다. 문제는 관리자 계정과 IP 데이터를 처음에 불러오는 코드에서 계속 NPE가 나는 것이다.
...
1. **`@DependsOn`**
    ...
    - 특정 빈이 다른 빈보다 먼저 생성되도록 순서를 강제할 수 있다.
