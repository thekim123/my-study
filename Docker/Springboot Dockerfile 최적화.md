집에서 ecs에 `Docker` 이미지를 올리는데 너무 느리다 못해 중간에 꺼져서(우리집 인터넷은 많이 느리다.) 이유를 파악하다가 이 `Docker` 이미지 크기 자체가 1GB에 육박하는 것을 발견했다. 그래서 이 용량을 줄이기 위해 여러가지 시도를 하고 그 기록을 적는다. 원래는 950MB였었던 용량이 417MB로 낮아졌다. 44%가 된 것이다. 내 나름에는 꽤 괜찮은 성과를 거뒀다고 생각한다.
